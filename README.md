...
![version](https://img.shields.io/badge/version-3.26-blue)
...
| Run-Ahead Mode | `run_ahead_secondary_instance = "false"` | Single instance, ~½ CPU cost. FBN pins `"true"` per FBN core maintainer recommendation; other Tier 1 cores inherit the global `"false"` |
...
| Security | On-Demand Thumbnails | `network_on_demand_thumbnails = "false"` | DiD pin on outbound thumbnail-server traffic; blocks unsolicited fetches against playlist scans |
...
| Input | Max Users | `input_max_users = "4"` | Caps player slots at 4 (matches Mupen pak1-4 rumble parity); flip from non-DINGUX upstream default 8. tvOS RA hard-cap is 3 per [#16685](https://github.com/libretro/RetroArch/issues/16685); 4th slot is forward-compat headroom if cap raised |
...
| Audio | Resampler Quality | `audio_resampler_quality = "3"` | NORMAL per `libretro-common/include/audio/audio_resampler.h` enum (DONTCARE=0, LOWEST=1, LOWER=2, NORMAL=3, HIGHER=4, HIGHEST=5). Real flip from tvOS upstream LOWER default (`config.def.h __MACH__ && IOS` branch) up to non-mobile NORMAL — SINC interpolation vs LOWER's linear; audible quality bump on cores not native to 48 kHz (NES/SNES 32040 Hz, GEN 53267 Hz all resample to the 48000 sink) at sub-1% A15 CPU cost |
...
|------|--------|------|----------|-------|
| 1 | NES | Mesen | Yes | Overscale 224p; per-game `mesen_overclock` (enum: None / Low / Medium / High / Very High) for Battletoads, Recca |
| 1 | SNES | Snes9x | Yes | Overscale 224p; per-game `snes9x_overclock_superfx = "200%"` for SuperFX titles (Star Fox, Yoshi's Island, Doom, Stunt Race FX) |
...
| 3 | Mupen64Plus-Next per-core rewind config (feature request) | [#18300](https://github.com/libretro/RetroArch/issues/18300) | Closed (completed; per-core rewind landed) | Per-core `rewind_enable = "false"` pin retained as drift-guard against per-game re-enable |
| 4 | Mupen64Plus-Next auto frame delay incompatible | [#14201](https://github.com/libretro/RetroArch/issues/14201) | Closed (completed) | `video_frame_delay_auto = "false"` per-core pin retained as drift-guard |
| 5 | N64 rendering glitches (game-specific) | [#16598](https://github.com/libretro/RetroArch/issues/16598) | Closed (completed) | Per-game overrides for any residual title-specific glitches |
| 6 | Threaded video force-disabled on all Apple platforms | [#14978](https://github.com/libretro/RetroArch/issues/14978) | Closed (force-disabled upstream) | `gfx/video_driver.c` `#if defined(__MACH__) && defined(__APPLE__)` runtime override; `video_threaded=false` globally; Tier 2 cfgs pin |
